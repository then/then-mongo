...
function Database(_parent) {
  ...
  if (!("ObjectId" in _parent)) throw new Error("Missing required property ObjectId");
...
Database.prototype.runCommand = function runCommand(opts) {
  var _parent = this._parent
  return new Promise(function (resolve, reject) {
    _parent.runCommand(opts, function (err, res) { err ? reject(err) : resolve(res) });
  });
};
