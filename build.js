...
fs.readdirSync(__dirname + '/schema').forEach(function (type) {
  ...
  var proxy = [];
  var typedef = [];
  var both = {push(str) { proxy.push(str); typedef.push(str); }}
  both.push('/**');
  both.push(' * WARNING: This file is autogenerated by `build.js` from in `/schema/`' + type);
  both.push(' */');
  both.push('');
  ...
  if (schema.methods) {
    Object.keys(schema.methods).filter(function (method) {
      ...
      proxy.push('var ' + name + ' = require("./' + name.toLowerCase() + '");');
  ...
  if (schema.methods) {
    Object.keys(schema.methods).forEach(function (name) {
      ...
      const args = method.args.map(arg => {
        return `${arg.name}${('default' in arg) ? '?' : ''}: ${arg.type || 'any'}`;
      }).join(', ');
      if (method.desc) {
        typedef.push(`  /**`);
  ...
  fs.writeFileSync(__dirname + '/lib/' + type.replace(/\.js$/, '.d.ts'), typedef.join('\n'));
});
...
classes.forEach(c => {
  indexTypeDef.push(`import ${c.className} from './lib/${c.type.replace(/\.js$/, '')}';`);
});
indexTypeDef.push(``);
...
indexTypeDef.push(` */`);
indexTypeDef.push(`function connect(connString: string, collectionNames?: Array<string>, options?: Object): Database;`);
