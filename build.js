...
fs.readdirSync(__dirname + '/schema').forEach(function (type) {
  ...
  var original = require('mongojs/lib/' + type);
  // TODO: check schema against original.prototype
  var proxy = [];
  proxy.push('/**');
  ...
  if (schema.properties) {
    schema.properties.forEach(function (property) {
      proxy.push('  if (!("' + property + '" in _parent)) throw new Error("Missing required property ' + property + '");');
      proxy.push('  this.' + property + ' = _parent.' + property + ';');
    });
  }
  proxy.push('}');
  proxy.push('');
